...
public class StudentApp {
	public static void main(String[] args) {
		...
		/**
		 * students[0] = new Student("23-001","홍길동",77,88); students[1] = new
		 * Student("23-002","송강",82,85);
		 */
		...
		try (Scanner scn = new Scanner(System.in)) {
			...
			while (run) {
				...
				case 3: // 단건조회
					...
					no = scn.nextLine();
					Student stnt = dao.getStudnet(no);
					if (stnt != null) {
						stnt.showInfo();
					} else {
					...
					/**
					 * for(int i = 0; i < students.length;i++) { if(students[i] != null // &&
					 * students[i].getStudNo().equals(no)) { students[i].showInfo(); } } //조회된 정보가
					 * 없습니다 exist;
					 */
					break;
				case 4:// 수정.
